    ...
    public void handle(CommandContext ctx) throws InterruptedException, IOException, SQLException {
        ...
        if (args.isEmpty()) {
            ...
            channel.sendMessageEmbeds(embedBuilder.build()).setActionRows(
                    ActionRow.of(
                            net.dv8tion.jda.api.interactions.components.Button.secondary(ctx.getMember().getUser().getId() + ":spam", "Spam").withEmoji(Emoji.fromEmote("spam", Long.parseLong("862895295239028756"), true)))).queue();
